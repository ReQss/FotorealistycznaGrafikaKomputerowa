...
// -------------------------------------------------------
// Model Phonga z promieniem cienia
// -------------------------------------------------------
Color phong(const HitRecord& rec,
            const Ray& ray,
            const PointLight& light,
            ...
            const vector<Plane>&  planes)
{
    ...
    {
        auto hit = sphere.intersects(shadowRay);
        if (hit && hit->t < dist) { inShadow = true; break; }
    }
    if (!inShadow)
    {
        for (const auto& plane : planes)
...
int main()
{
    ...
    // Material podlogi – szary, matowy, slaby specular
    Material matFloor(
    ...
    // Podloga pod sferami: y = -1.2 (dotyka dolnej sfery niebieskiej)
    vector<Plane> planes = {
        Plane(Vector3(0, -1.2, 0), Vector3(0, 1, 0), matFloor)
    };

    // Zrodlo punktowe – z gory i z boku, zeby cien byl dobrze widoczny
    PointLight light(
        Vector3(4.0, 6.0, -2.0),
        Color(1.0, 1.0, 1.0),
        1.0, 0.04, 0.002
